...
class ChannelService
{
    ...
    public function formattedChannel(Channel $channel) {
        $channel['partners'] = $channel->users->filter(fn ($user) => $user->id != Auth::user()->id)->values();
        $channel['preview'] = $channel->messages->isEmpty() ? '': $channel->messages->last();
        return $channel;
    }

    public function formattedChannels($authUser) {
